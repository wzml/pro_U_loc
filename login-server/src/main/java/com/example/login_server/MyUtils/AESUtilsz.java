...
public class AESUtilsz {
    ...
    private static final String CiperMode = "AES/CBC/PKCS5Padding";  //  加密用的key可由26个字母和数字组成，此处使用AES-128—CBC加密模式，key需要为16位

    public String encrypt(String encData,String key)throws Exception{
        try{
            Cipher cipher = Cipher.getInstance(CiperMode);
            byte[] raw = genkey(key);  //  生成16位长的密钥
            SecretKeySpec skeySpec = new SecretKeySpec(raw,"AES");
            IvParameterSpec iv = new IvParameterSpec(ivParameter.getBytes());  //  使用CBC模式，需要一个向量iv，可增加加密算法的强度
            cipher.init(Cipher.ENCRYPT_MODE,skeySpec,iv);
            byte[] encrypted = cipher.doFinal(encData.getBytes());
            return Base64.encodeToString(encrypted,Base64.DEFAULT);  //  使用BASE64做转码
        }catch (Exception e){
            e.printStackTrace();
            return null;
        }
